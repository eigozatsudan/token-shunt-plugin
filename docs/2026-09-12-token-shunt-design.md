...
- 各委譲・再試行・境界確認のプロンプトに、パス付き事実・値だけを返す出力契約も添える。定義や呼出関係は短い説明と識別子で表し、逐語証拠を求められても代入行・関数本体を引用しない。送信前にコードフェンスが無いこと、各要求事実が confirmed または unconfirmed であることを確認する。
...
|---|---|---|---|---|
| compare-bulk-facts | `report_token()` が返す文字列と、その供給元関数名・定義開始行（return でなく def 行） | 400 行超かつ 64KiB 超。行 20 付近と行 380 付近に互いに参照する識別子 | 値・関数名・行番号 | 直接 + 委譲（委譲プロンプトにスキル指示あり） |
...
- 再試行前に満たされていない要求を特定する。パス付き事実と要求値は根拠として足り、逐語引用の欠如や関数名からの疑念だけでは昇格しない。再試行には元の質問・同じ指定パスとメタデータ・出力契約・`retry_reason: missing required evidence|response contract violation|verification failed` の該当理由と具体的な不足事実または失敗条件を渡す。引用要求で補わせない。§11 のバッチ境界確認は別の確認起動であり、同じパス集合を使うこの昇格規則とは区別する。いずれも総起動上限に含める。Haiku 履歴は渡さないが、同じファイルを読む費用は再度かかる。両試行を計上する。
